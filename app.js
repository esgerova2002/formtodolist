...
document.addEventListener("DOMContentLoaded",function(){
    const yeniRolbutton=document.querySelector('.topbutton button');
    const formsil=document.querySelector('.bottombutton #imtina');
    const yaddaSaxla=document.querySelector('.bottombutton #yaddas');
    ...
    const formHTML = `
     ...
     yeniRolbutton.addEventListener('click',function(){
        ...
        yeniDiv.innerHTML=formHTML;
        document.body.appendChild('yeniDiv');
        formsil.addEventListener('click',function(){
            yeniDiv.remove();
        })  ;
        yaddaSaxla.addEventListener('click',function(){
            const name=document.querySelector('input[name="name"]').value;
            const description=document.querySelector('input[name="description"]').value;
            const cat=document.querySelector('select[name]="category');
            if(name && description && cat!=='secin'){
                const newDiv=document.createElement(div);
                newDiv.classList.add("card");
                newDiv.innerHTML=`<p>${name}</p><p>${description}</p>`
                if(cat=='sistem'){
                    sistemRol=document.querySelector('.sistemRollari .cards'){
                        document.appendChild("sistemRol")
                    }
                }
            }
        })
